...
var reset={total:0,shown:0,mirror:[],current:[],tag:'',searching:false};
var options={
	debug:1,
	theme:0,
	limit:12,
...
document.addEventListener('DOMContentLoaded',function(){
	...
	DOM.specials.onclick=function(){
		...
		options.mirror=options.current.slice(0);
		options.index=Object.keys(database);
		_clear(DOM.list);
		DOM.done.innerHTML='<i class="fa fa-eye"></i>Просмотренное';
		_load();
		_quote();
	};
	...
	DOM.showmode.onmousedown=function(){
		options.showmode=(options.showmode?0:1);
		_apply('#list article',function(element){
			if(options.showmode){ element.classList.add('sort'); }
			else { element.classList.remove('sort'); }
		});
	};
	...
	_apply('#sortlist .filter',function(element){
		element.onclick=function(event){
			_clear(DOM.list);
			var f=parseInt(element.dataset.filter), d=parseInt(element.dataset.direction);
			if(options.mirror.length==0) options.mirror=options.index.slice(0);
			if(options.sort.filter==f){ d=(d?0:1); element.dataset.direction=d; _filter(f,d); } // filter already applied
			else { options.sort.filter=f; _filter(f,d); }
			options.current=options.mirror.slice(0);
			options.total=options.mirror.length;
			options.shown=0;
			options.sort.enable=1;
			_load();
			_quote();
		}
	});
...
function _filter(type,dir){
	var crit=''; switch(type){
		case 1: crit='duration'; break;
		case 2: crit='rating'; break;
		case 4: crit='budget'; break;
...
function _reset(){ console.info('call reset');
	database=databaseNew; options.view=0; options.sort.enable=0;
	for(var o in reset){ options[o]=reset[o]; }
	options.total=options.index.length;
	options.index=Object.keys(database);
	options.current=options.index.slice(0);
...
function _load(tag){
	_loading(1);
	DOM.error.cloak();
	if(options.index.length==0){ _error('Ошибка при подгрузке: индекс пуст, показывать нечего.','Zero index @ _load()'); return false; }
	if(options.shown==0 && !options.searching && !options.sort.enable){ options.current=options.index.slice(0); }
	if(!options.mirror) options.mirror=options.current.slice(0);
	if(tag!==undefined){
		_reset();
		options.tag=tag;
		options.current=[];
	...
	if(options.view==2){ _error('Рубрика «Специальные предложения» — доставляется непосредственно Анонимусом!'); }
	if(window.Promise!==undefined){
		var go=new Promise(function(resolve,reject){ // здесь мы только выводим кусок options.current (считая с начала и с лимитом options.limit)
			if(options.current.length==0){ reject('(empty index)'); return false; }
			var j=options.limit;
				if(j<=0) j=options.index.length;
				if(j>=options.current.length) j=options.current.length;
			var entry=0,i=0;
			if(options.sort.enable){
				for(i=0; i<j; i++) if(!_show(options.current[i])){ reject('Unable to show entry'+i); return false; }
				//for(i=0; i<j; i++) options.current.remove(0); // @todo: replace with slice
				options.current.remove(options.current.splice(0,j));
			}
			else for(i=0; i<j; i++){
...
function _search(keywords){
	...
	else { options.mirror=options.current.slice(0); _load(); }
}
...
function _show(id){
	...
	var article=_create('article');
		article.id=id;
		article.classList.add('smooth'); if(options.showmode) article.classList.add('sort');
		...
		var expandT=_create('div');
			...
			var expand=_create('div');
				...
				var F_about=_create('div');
					...
					var F_anonrate=_create('div');
					if(options.view==1 && data.anonrate){
						F_anonrate.innerHTML=data.anonrate;
						F_anonrate.classList.add('full-anonrate');
					}
					var F_special=_create('div');
					if(options.view==2 && data.special){
						F_anonrate.innerHTML=data.special;
						F_anonrate.classList.add('full-special');
					}
					var F_desc=_create('div');
						F_desc.innerHTML=data.description;
						F_desc.classList.add('full-desc');
					F_about.appendChilds(F_title,F_subtitle,F_year,F_duration,F_genre,F_country,F_budget,F_rating,F_anonrate,F_desc); // закурил
...
var quotes=[
	...
	'Tanz mit mir, tanz mit mir, schöne Komsomolka!',
	'Ваша утка в нашем дупле',
	'РАМИТЕП!',
	'У нас тут все окна на юг',
	'Упыри',
	'Дипломированные психи'
];
